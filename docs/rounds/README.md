...
| 2026-09-04 | P1/P2 方法学修复落码 | 修复 | R-7 质心惯量语义（双向 Steiner）+ 三角不等式检查；multiple shooting/延迟回放/通道归一化/零模型 bar （修 R-1 90.2% 独占与地板退化）；ACTUATOR R²≥0.80 过滤带 [0.546,0.706]；官方区间视图 identified_intervals.json（κs/m 0.113-0.117）；新门禁远端重跑证据后被 2026-09-08 轮闭环（见下） | [2026-09-04_sysid_review.md §13](2026-09-04_sysid_review.md) |
...
- ⚠️ **使用 R9 参数前必读** [2026-09-04_sysid_review.md](2026-09-04_sysid_review.md)：事后复算显示 m 与 κs 结构性简并（可辨识量是 **κs/m ≈ 0.113–0.117**，非 m 或 κs 各自），骨盆惯量与质心证据不足（建议暂缓回写，§9）；T9 门禁判定本身不变。**2026-09-08 新门禁复验（TASK_20260908_225）已代码级确认：R9 在 P1-4/P2-1 新门禁下 3/5 PASS、整体 FAIL（ACCEL 13.541 vs bar 2.423；ACTUATOR κs 出带）**——R9 不满足当前门禁体系，后续任何再辨识必须在新门禁下重跑
