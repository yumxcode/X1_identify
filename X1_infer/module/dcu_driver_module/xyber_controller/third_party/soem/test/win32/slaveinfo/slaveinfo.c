    ...
    for (iSM = 2; iSM <= nSM; iSM++) {
      ...
      if (wkc > 0) {
        ...
        if (tSM == 3)  // outputs
        {
          ...
          printf(
              "  SM%1d outputs\n     addr b   index: sub bitl common_type    "
              "name\n",
              iSM);
          Tsize = si_PDOassign(
      ...
      if (PDO->SyncM[PDO->nPDO] < EC_MAXSM) /* active and in range SM? */
      {
        ...
        if (t)
          ...
          printf("  SM%1d TXPDO 0x%4.4X %s\n", PDO->SyncM[PDO->nPDO],
                 PDO->Index[PDO->nPDO], str_name);
        printf("     addr b   index: sub bitl common_type    name\n");
    ...
    if (ec_config(FALSE, &IOmap) > 0) {
      ...
      for (cnt = 1; cnt <= ec_slavecount; cnt++) {
        ...
        printf(" Man: %8.8x ID: %8.8x Rev: %8.8x\n", (int)ec_slave[cnt].eep_man,
               (int)ec_slave[cnt].eep_id, (int)ec_slave[cnt].eep_rev);
        for (nSM = 0; nSM < EC_MAXSM; nSM++) {
          if (ec_slave[cnt].SM[nSM].StartAddr > 0)
            printf(
                " SM%1d A:%4.4x L:%4d F:%8.8x Type:%d\n", nSM,
                ec_slave[cnt].SM[nSM].StartAddr, ec_slave[cnt].SM[nSM].SMlength,
        ...
        for (j = 0; j < ec_slave[cnt].FMMUunused; j++) {
          printf(
              ...
              "Act:%2.2x\n",
              j, (int)ec_slave[cnt].FMMU[j].LogStart,
              ec_slave[cnt].FMMU[j].LogLength,
              ec_slave[cnt].FMMU[j].LogStartbit,
              ec_slave[cnt].FMMU[j].LogEndbit, ec_slave[cnt].FMMU[j].PhysStart,
